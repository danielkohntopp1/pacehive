...
function buildChecklist(guide: Guide, hasAvatar: boolean): ChecklistItem[] {
  return [
    { done: hasAvatar, key: 'addPhoto', href: '/guia/perfil' },
    { done: (guide.run_types?.length ?? 0) > 0, key: 'selectRunTypes', href: '/guia/perfil' },
    { done: (guide.services?.length ?? 0) > 0, key: 'addServices', href: '/guia/perfil' },
    { done: !!guide.bio, key: 'writeBio', href: '/guia/perfil' },
    { done: !!guide.strava_stats, key: 'connectStrava', href: '/guia/perfil' },
    {
...
export default async function GuiaDashboard() {
  ...
  return (
    <div className="max-w-4xl">
      ...
      {!profileComplete && (
        <div className="bg-white rounded-2xl border border-[#E5E5E5] p-6 mb-8">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-base font-bold text-[#1A1A1A]">{t('completeProfile')}</h2>
              <p className="text-sm text-[#6B6B6B] mt-0.5">
                {t('itemsCompleted', { done: doneCount, total: totalCount })}
              </p>
            </div>
              ...
              >
                ...
                <div className="min-w-0">
                  <p className={`text-sm font-medium ${item.done ? 'line-through text-[#6B6B6B]' : 'text-[#1A1A1A]'}`}>
                    {t(`checklist.${item.key}.label`)}
                  </p>
                  {!item.done && <p className="text-xs text-[#6B6B6B]">{t(`checklist.${item.key}.hint`)}</p>}
                </div>
                {!item.done && (
                  <span className="ml-auto text-xs text-[#F5A623] font-semibold flex-shrink-0">{t('complete')}</span>
                )}
              </Link>
            ))}
          </div>
      ...
      {active.length > 0 && (
        <div className="mb-6">
          <h2 className="text-sm font-semibold text-[#6B6B6B] uppercase tracking-wider mb-3">{t('confirmed')}</h2>
          <div className="space-y-3">
      ...
      {bookings.length === 0 && profileComplete && (
        <div className="bg-white rounded-2xl border border-[#E5E5E5] p-12 text-center">
          ...
          <p className="text-[#6B6B6B] text-sm">{t('profileCompleteAndVisible')}</p>
        </div>
      )}
    </div>
  )
