...
export default async function RunnerBookingPage({ params }: Props) {
  ...
  return (
    <div className="max-w-2xl">
      <Link href="/dashboard" className="inline-flex items-center gap-1.5 text-sm text-[#6B6B6B] hover:text-[#1A1A1A] mb-6">
      ...
      <div className="bg-white rounded-2xl border border-[#E5E5E5] p-6 mb-4">
        ...
        {b.notes && (
          <div className="mt-4 p-3 bg-[#F9F5EE] rounded-xl">
            <p className="text-xs font-medium text-[#6B6B6B] mb-1">Observações</p>
        ...
        {b.status === 'accepted' && b.guide?.profile && (
          <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-xl">
            <p className="text-sm font-semibold text-green-800 mb-2">Corrida confirmada! Dados do guia:</p>
            <p className="text-sm text-green-700">E-mail: <strong>{b.guide.profile.email}</strong></p>
            {b.guide.profile.phone && (
              <p className="text-sm text-green-700">WhatsApp: <strong>{b.guide.profile.phone}</strong></p>
            )}
          </div>
        )}
      </div>
      ...
      {b.status === 'completed' && !existingReview && b.guide && (
        <div className="bg-white rounded-2xl border border-[#E5E5E5] p-6">
          <h2 className="text-lg font-bold text-[#1A1A1A] mb-4">Avalie sua experiência</h2>
          <ReviewForm
            bookingId={b.id}
